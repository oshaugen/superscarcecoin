...
#if defined(NDEBUG)
# error "Superscarcecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2c0f91cdf56841b5665623b0e0ab2451ff28f99937432679e4ac4905799e10de");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Superscarcecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Superscarcecoin Signed Message:\n";
...
{
    // Superscarcecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 250000); // Superscarcecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Superscarcecoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Superscarcecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Superscarcecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Superscarcecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x342e4dfe89e395fd0dceccc6330913decc5fd250d7fe32018b72bee94af0a8f7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 31/Dec/2023 In debt limit fight, reps wont say what spending cuts they want";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f0e75de8b85da5e1d7da7c3acec88e2bce59f27e67b340cca82a4eeef3a0ce20dfff59d302a7c679e7f3357bb7347aed447f61dd595dacf8aecd98f7bf2e62e3") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1675199983;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084795552;

        if (fTestNet)
        {
            block.nTime    = 1675199944;
            block.nNonce   = 385669808;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x3ef6b70ea26a222450adff5638e717b6562d19d5880412b97ca910092f8fa32f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xc9, 0xba, 0xd1 }; // Superscarcecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SuperscarcecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SuperscarcecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SuperscarcecoinMiner(CWallet *pwallet)
{
    printf("SuperscarcecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("superscarcecoin-miner");
    ...
    try { loop {
        ...
        printf("Running SuperscarcecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SuperscarcecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SuperscarcecoinMiner, pwallet));
}
